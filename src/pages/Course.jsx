  ...
  async function handleOnSubmit() {
    if (formData.courseName && formData.yearLevel) {
      if (selectedCourseId === null) {
        ...
        dispatch(
          updateCourse({ courseId: selectedCourseId, formData: formData })
        );
        toast.success(
          formData.courseName +
            " - " +
              ...
              {!courses
                ...
                : courses.map((course) => {
                    ...
                    return (
                      <tr
                        key={_id}
                        onClick={() => handleSubjectClick(_id)}
                        id="qweqweqwe"
                        className="border-b bg-gray-700 border-gray-700 hover:cursor-pointer hover:bg-gray-500"
                      >
                        <td className="py-2 px-3">{courseName}</td>
                        <td className="py-2 px-3 text-center">{yearLevel}</td>
      ...
      >
        <div className="m-4 space-y-4 p-10 flex flex-col">
          ...
          <div className="flex flex-col">
            ...
            <select
              onChange={handleOnChange}
              className="bg-gray-600 mb-5 text-gray-200 py-2 px-3 rounded-md"
              name="yearLevel"
              id="yearLevel"
            ...
            >
              ...
              <option value="I" className="text-gray-200 p-2 text-lg">
                I
              </option>
              <option value="II" className="text-gray-200 p-2 text-lg">
                II
              </option>
              <option value="III" className="text-gray-200 p-2 text-lg">
                III
              </option>
              <option value="IV" className="text-gray-200 p-2 text-lg">
                IV
              </option>
            </select>
          </div>
          <button
            ...
            className="bg-indigo-800 py-2 px-4 active:bg-indigo-900 rounded-md"
          >
            Save
          </button>
        </div>
